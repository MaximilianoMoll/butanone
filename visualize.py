        ...
        for col, data in enumerate(dataset):
            ...
            for i, method in enumerate(methods):
                ...
                axis[row, col].plot(
                    ...
                    sub[metric],
                    color=lcolors[i],
                    label=sub["method"][0].decode("utf-8"),
                )
        ...
        for row, data in enumerate(dataset):
            ...
            for i, method in enumerate(methods):
                ...
                axis[col, row].plot(
                    ...
                    sub_data3["anon_f1"],
                    color=lcolors[i],
                    label=sub_data3["method"][0].decode("utf-8"),
                )
...
def run_anon_data_ml():
    ...
    for data in dataset:
        ...
        ATT_TREES = read_tree(gen_path, data, ATT_NAMES, QI_INDEX, HAS_HIERARCHIES)
        ...
        for classifier_name in ml_metrics:
            ori_f1 = classifier_evaluation(
                classifier_name, ori_csv, train_index, val_index, QI_INDEX, IS_CAT
            )
            for method in methods:
                for k in k_array:
                    anon_csv = os.path.join(
                        result_path, data, method, f"{data}_anonymized_{k}.csv"
                    )
